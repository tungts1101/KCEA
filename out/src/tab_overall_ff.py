"""Write FF standalone table and body snippet to assets/tables/.

Run from KCEA/:
    python -m out.src.tab_overall_ff [--input result.json] [--out-dir assets/tables]
"""
...
CAPTION_STANDALONE = (
    r"Final Forgetting (FF, \%) on seven CIL benchmarks. "
    r"Lower is better. "
    r"Mean $\pm$ std across three seeds (1993, 1994, 1995). "
    r"Best (lowest) in \textbf{bold}, second best in \textit{italic}."
)
LABEL_STANDALONE = "tab:overall-ff-standalone"
...
def main(input_path: Path = DEFAULT_INPUT, out_dir: Path = DEFAULT_OUT_DIR) -> None:
    ...
    body = build_body(agg, metric="FF", lower_is_better=True)

    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    body_path = out_dir / "tab_overall_ff_body.tex"
    body_path.write_text(body)
    print(f"  [saved] {body_path}")

    standalone_path = out_dir / "tab_overall_ff.tex"
