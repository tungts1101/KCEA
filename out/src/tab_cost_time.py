"""Generate wall-clock time cost table.

Run from KCEA/:
    python -m out.src.tab_cost_time \\
...
from out.src.cost_table import build_time_body, COST_BASELINE_KEYS, COST_KCEA_KEY
...
CAPTION = (
    r"Wall-clock time cost comparison (cumulative across all tasks). "
    r"Mean $\pm$ std across datasets. "
    r"Best (lowest) training time per column in \textbf{bold}. "
...
def main(
    ...
    body = build_time_body(costs, COST_BASELINE_KEYS, COST_KCEA_KEY)

    body_path = out_dir / "tab_cost_time_body.tex"
    body_path.write_text(body)
    print(f"  [saved] {body_path}")

    standalone_path = out_dir / "tab_cost_time.tex"
